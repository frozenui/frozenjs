...
!function ($) {
	...
	$.attachObject=function(el,obj,tpl,option){
		
		if($.isArray(el)&&el[0].nodeName.toLowerCase()!="script"&&el.selector!=""){
			return el.each(function () {
				var $this = $(this);
				
				var data  = $this.data('fz.'+obj.role);
				if (!data) $this.data('fz.'+obj.role, (data = new obj(this,option)));
				if (typeof option == 'string') data[option].call(this);
			})
		}else{
			var htmlTpl=tpl||"";
			if($.isArray(el)){
				htmlTpl=el.selector!=""?$(el[0]).html():el[0].outerHTML;
			}
			htmlTpl=htmlTpl.replace(/&lt;/g,"<").replace(/&gt;/g,">");
			
			var html=$.tpl(htmlTpl,option||{});
			$(html).data("role",obj.role);
			var data=new obj($(html)[0],option);
			if (typeof option == 'string') data[option].call($(html)[0]);
			return $(html);
		}
		return null;
	}
